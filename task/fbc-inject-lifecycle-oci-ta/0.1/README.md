...
Lifecycle injection only runs if **all** targeted OCP versions in the Dockerfile are >= 5.0.

- If any version is below 5.0, the component is not eligible, and the task exits successfully with no packages injected.
- If the component is eligible but no packages requiring lifecycle injection are found, the task fails.

### TEST_OUTPUT
The task writes a `TEST_OUTPUT` result following the Konflux convention. A `FAILURE` result is written if the eligibility check fails, no packages are found for an eligible component, `plcc2fbc` generation fails, or lifecycle injection fails, allowing Conforma to evaluate the result without halting the pipeline. A `SUCCESS` result with zero successes is written if the component is not eligible for lifecycle injection.
